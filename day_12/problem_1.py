...
# check shortest based on the path lengths connected to "S"
min_length = no_go_length
# check upwards
check_loc = (start_loc[0] - 1, start_loc[1])
if check_loc[0] >= 0 and path_lengths[check_loc] < min_length:
    min_length = path_lengths[check_loc]
# check downwards
check_loc = (start_loc[0] + 1, start_loc[1])
if check_loc[0] < grid.shape[0] and path_lengths[check_loc] < min_length:
    min_length = path_lengths[check_loc]
# check left
check_loc = (start_loc[0], start_loc[1] - 1)
if check_loc[1] >= 0 and path_lengths[check_loc] < min_length:
    min_length = path_lengths[check_loc]
# check downwards
check_loc = (start_loc[0], start_loc[1] + 1)
if check_loc[1] < grid.shape[1] and path_lengths[check_loc] < min_length:
    min_length = path_lengths[check_loc]

# this didn't include S stepping to the best path square so we add 1
min_length += 1
